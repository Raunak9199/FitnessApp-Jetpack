package com.nexusbihar.fitnessapp.data.services
...
class ExportService(private val context: Context) {
    ...
    suspend fun exportToCSV(healthData: List<HealthData>): String = withContext(Dispatchers.IO) {
        ...
        "Downloads/FitnessApp_Data_$timestamp.csv"
    }
...
//                    .setBold()
        ...
        // Return user-friendly path
        "Downloads/FitnessApp_Report_$timestamp.pdf"
    }
    ...
    fun getAvailableExports(): List<File> {
        ...
        return downloadsDir.listFiles { file ->
            file.name.startsWith("FitnessApp_") && (file.name.endsWith(".csv") || file.name.endsWith(".pdf"))
        }?.toList() ?: emptyList()
    }
